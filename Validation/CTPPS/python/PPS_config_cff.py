...
def _modify2016(process):
    ...
    process.load('Validation.CTPPS.year_2016_cfi')

def _modify2017(process):
    print('Process customised for 2017 PPS era')
    process.load('Validation.CTPPS.year_2017_cfi')

def _modify2018(process):
    print('Process customised for 2018 PPS era')
    process.load('Validation.CTPPS.year_2018_cfi')

def _modify2021(process):
    print('Process customised for 2021 PPS era')
    process.load('Validation.CTPPS.year_2021_cfi')
    if hasattr(process, 'generator'):
        process.generator.energy = process.profile_2021_default.ctppsLHCInfo.beamEnergy
...
def _modify2022(process):
    ...
    process.load('Validation.CTPPS.yea_2022_cfi')
    if hasattr(process, 'generator'):

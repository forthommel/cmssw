...
CTPPSRPAlignmentCorrectionsMethods::loadFromXML( const std::string& fileName )
{
  // prepare output
  ...
  try {
    ...
    throw cms::Exception("CTPPSRPAlignmentCorrectionsMethods") << "An XMLException caught with message: " << cms::xerces::toString( toCatch.getMessage() ) << ".";
  }
  ...
  if ( !parser )
    throw cms::Exception("CTPPSRPAlignmentCorrectionsMethods") << "Cannot parse file `" << fileName << "' (parser = NULL).";

  DOMDocument* xmlDoc = parser->getDocument();

  if ( !xmlDoc )
    throw cms::Exception("CTPPSRPAlignmentCorrectionsMethods") << "Cannot parse file `" << fileName << "' (xmlDoc = NULL).";
      ...
      if ( nodeType == 0 )
        ...
        for ( unsigned int j = 0; j < attrs->getLength(); j++ )
        {
          ...
          if ( attr_name == "first" )
          {
            ...
            edm::LogProblem("CTPPSRPAlignmentCorrectionsMethods") << ">> CTPPSRPAlignmentCorrectionsDataSequence::loadFromXML > Warning: unknown attribute `"
              << attr_name << "'.";
        }

        // interval of validity must be set
        if ( !first_set || !last_set )
          throw cms::Exception("CTPPSRPAlignmentCorrectionsMethods") << "iov tag must have `first' and `last' attributes set.";
...
CTPPSRPAlignmentCorrectionsMethods::getCorrectionsData( DOMNode* root )
{
  ...
  for ( unsigned int i = 0; i < children->getLength(); i++ )
    {
      ...
      if ( nodeType == 0 )
	      throw cms::Exception("CTPPSRPAlignmentCorrectionsMethods") << "Unknown node `" << cms::xerces::toString( node->getNodeName() ) << "'.";

      // check children
      if ( node->getChildNodes()->getLength() > 0 )
	    {
	      edm::LogProblem("CTPPSRPAlignmentCorrectionsMethods") << "LoadXMLFile > Warning: tag `" <<
      ...
      for ( unsigned int j = 0; j < attr->getLength(); j++ )
	    {
	      ...
	      if ( node_name == "id" )
	      {
	        ...
	        edm::LogProblem("CTPPSRPAlignmentCorrectionsMethods") << ">> CTPPSRPAlignmentCorrectionsMethods::getCorrectionsData > Warning: unknown attribute `"
							     << cms::xerces::toString( a->getNodeName() ) << "'.";
	   }

      // id must be set
      if ( !idSet )
        throw cms::Exception("CTPPSRPAlignmentCorrectionsMethods") << "Id not set for tag `" << cms::xerces::toString( node->getNodeName() ) << "'.";

      // build alignment
      const CTPPSRPAlignmentCorrectionData align_corr(
...
void
CTPPSRPAlignmentCorrectionsMethods::writeToXML( const CTPPSRPAlignmentCorrectionsDataSequence &data, const std::string& fileName, bool precise, bool wrErrors,
  bool wrSh_xy, bool wrSh_z, bool wrRot_xy, bool wrRot_z )
{
  FILE* rf = fopen( fileName.c_str(), "w" );
  if ( !rf )
    throw cms::Exception("CTPPSRPAlignmentCorrectionsMethods") << "Cannot open file `" << fileName << "' to save alignments.";
...
void
CTPPSRPAlignmentCorrectionsMethods::writeXMLBlock( const CTPPSRPAlignmentCorrectionsData& data, FILE* rf, bool precise, bool wrErrors,
					      bool wrSh_xy, bool wrSh_z, bool wrRot_xy, bool wrRot_z )
{
  bool firstRP = true;
...
void
CTPPSRPAlignmentCorrectionsMethods::writeXML( const CTPPSRPAlignmentCorrectionData& data, FILE* f, bool precise, bool wrErrors,
					 bool wrSh_xy, bool wrSh_z, bool wrRot_xy, bool wrRot_z )
{
  if ( wrSh_xy )
  {

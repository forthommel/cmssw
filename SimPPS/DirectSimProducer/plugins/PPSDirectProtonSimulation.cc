...
void PPSDirectProtonSimulation::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  edm::ESHandle<CTPPSBeamParameters> beamParameters;
  edm::ESHandle<PPSPixelTopology> ppt;
  edm::ESHandle<LHCInterpolatedOpticalFunctionsSetCollection> opticalFunctions;
  edm::ESHandle<CTPPSGeometry> geometry;
  edm::ESHandle<PPSDirectSimulationData> directSimuData;

  iSetup.get<LHCInfoRcd>().get(edm::ESInputTag{"", lhcInfoLabel_}, lhcInfo);
  iSetup.get<CTPPSBeamParametersRcd>().get(beamParameters);
  iSetup.get<PPSPixelTopologyRcd>().get(ppt);
  iSetup.get<CTPPSInterpolatedOpticsRcd>().get(edm::ESInputTag{"", opticsLabel_}, opticalFunctions);
  iSetup.get<VeryForwardMisalignedGeometryRecord>().get(geometry);
  iSetup.get<PPSDirectSimulationDataRcd>().get(directSimuData);
  ...
  for (const auto &ofp : opticalFunctions) {
    ...
    for (const auto &detIdInt : geometry.getSensorsInRP(rpId)) {
      ...
      if (detId.subdetId() == CTPPSDetId::sdTimingDiamond) {
        ...
        // check acceptance
        const auto *dg = geometry.getSensor(detIdInt);
